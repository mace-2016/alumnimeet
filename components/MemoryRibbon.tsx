...
export default function MemoryRibbon() {
  ...
  return (
    <div className="relative w-full overflow-hidden bg-white rounded-[2rem] md:rounded-[2.5rem] border border-[var(--border)] p-8 md:p-10 lg:p-12 shadow-[0_8px_30px_rgba(116,12,8,0.02)] group/bento">
      ...
      <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
        ...
        <Link href="/gallery" className="shrink-0 flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-full border border-[var(--border)] bg-white hover:border-[var(--color-mace-crimson)]/30 hover:bg-[var(--color-mace-crimson)]/5 transition-all duration-300 group/btn shadow-sm">
          <ArrowUpRight className="h-5 w-5 text-[var(--color-mace-stone)] group-hover/btn:text-[var(--color-mace-crimson)] transition-all" strokeWidth={1.5} />
        </Link>
      </div>

      {/* Ribbon Container - Removed negative margins that caused the clipping */}
      <div className="relative w-full flex flex-col items-center py-4">
        
        {/* Edge Masks */}
        <div className="absolute inset-y-0 left-0 w-20 md:w-40 z-20 bg-gradient-to-r from-white via-white/80 to-transparent pointer-events-none" />
        <div className="absolute inset-y-0 right-0 w-20 md:w-40 z-20 bg-gradient-to-l from-white via-white/80 to-transparent pointer-events-none" />
        ...
        <div 
          ...
          style={{ 
            ...
            /* Ensures hover still works when NOT manually paused */
          ...
          {displayImages.map((item, idx) => (
            ...
            >
              ...
              <div className="absolute bottom-4 left-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                 ...
                 <p className="text-[var(--color-mace-gold)] text-xs font-bold">{item.year}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <button
          ...
          className="mt-10 relative z-30 flex items-center gap-2 px-5 py-2 rounded-full border border-[var(--border)] bg-white hover:bg-[var(--surface-soft)] text-[var(--color-mace-stone)] transition-all shadow-sm active:scale-95"
        >
          {isPaused ? (
            <>
      ...
      <style jsx global>{`
        ...
        /* Only pause on hover if the user hasn't manually played it */
