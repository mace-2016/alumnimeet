  ...
  return (
    <div className="group/gallery relative w-full max-md:w-[calc(100%+3rem)] max-md:-mx-6 overflow-hidden bg-white max-md:bg-transparent rounded-[2rem] max-md:rounded-none md:rounded-[2.5rem] border border-[var(--border)] max-md:border-none p-8 max-md:p-0 md:p-10 lg:p-12 shadow-[0_8px_30px_rgba(116,12,8,0.02)] max-md:shadow-none transition-all duration-700 hover:shadow-[0_40px_80px_rgba(116,12,8,0.08)] hover:-translate-y-1.5 max-md:hover:translate-y-0">
      ...
      <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10 max-md:hidden">
        <div className="flex flex-col md:flex-row md:items-center gap-6 lg:gap-8">
          <div className="w-14 h-14 md:w-16 md:h-16 lg:w-20 lg:h-20 rounded-2xl md:rounded-[1.5rem] bg-[var(--surface-soft)] border border-[var(--border)] flex items-center justify-center shrink-0 transition-all duration-700 group-hover/gallery:bg-[var(--color-mace-gold)] group-hover/gallery:border-[var(--color-mace-gold)] group-hover/gallery:scale-105 shadow-sm">
            <Film className="h-7 w-7 md:h-8 md:w-8 lg:h-10 lg:w-10 text-[var(--color-mace-gold)] group-hover/gallery:text-white transition-colors duration-500" strokeWidth={1.5} />
          </div>
          <div>
        ...
        <Link href="/gallery" className="shrink-0 flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-full border border-[var(--border)] bg-white hover:border-[var(--color-mace-gold)]/20 hover:bg-[var(--color-mace-gold)]/10 transition-all duration-300 shadow-sm">
          <ArrowUpRight className="h-5 w-5 text-[var(--color-mace-stone)] group-hover/gallery:text-[var(--color-mace-gold)] group-hover/gallery:translate-x-0.5 group-hover/gallery:-translate-y-0.5 transition-all duration-500" strokeWidth={1.5} />
        </Link>
      </div>
      ...
      <div className="relative flex flex-col items-center -mx-8 md:-mx-10 lg:-mx-12 py-4 max-md:mx-0 max-md:py-0">
        ...
        <div className="flex w-max gap-4 md:gap-6 animate-ribbon-scroll px-8 md:px-10 lg:px-12 max-md:px-6" style={{ animationPlayState: isPaused ? 'paused' : 'running' }}>
          {displayImages.map((item, idx) => (
            <div key={idx} className="relative flex-shrink-0 w-[200px] h-[260px] md:w-[260px] md:h-[340px] overflow-hidden rounded-[1.5rem] border border-[var(--border)] shadow-sm transition-all duration-700 hover:-translate-y-2 hover:shadow-xl group/card">
        ...
        >
          {isPaused ? (
            ...
            <><Pause className="w-5 h-5 md:w-3 md:h-3 fill-current" /><span className="max-md:hidden text-[10px] font-bold uppercase tracking-[0.2em]">Pause</span></>
          )}
        </button>
      </div>

      {/* MOBILE ONLY FOOTER TEXT */}
      <div className="md:hidden mt-6 px-6 flex items-center justify-between w-full">
        <div>
          <h3 className="font-serif text-2xl font-medium tracking-tight text-[var(--color-mace-crimson)]">
            The <span className="gradient-gold font-semibold">Gallery</span>
          </h3>
        ...
        <Link href="/gallery" className="shrink-0 flex items-center justify-center w-10 h-10 rounded-full border border-[var(--border)] bg-white shadow-sm active:scale-95 transition-transform">
          <ArrowUpRight className="h-5 w-5 text-[var(--color-mace-stone)]" strokeWidth={1.5} />
        </Link>
      </div>
