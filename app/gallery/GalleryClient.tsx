...
// Define the expected shape of the incoming server data
...
export default function GalleryClient({ categories }: { categories: CategoryData[] }) {
              ...
              <div className="relative w-full">
                ...
                <div className="flex overflow-x-auto hide-scrollbar gap-4 md:gap-6 px-6 pb-8 snap-x snap-mandatory">
                  {category.locked ? (
                    ...
                    <>
                      {/* REAL DRIVE IMAGES */}
                      {category.images?.slice(0, 6).map((item) => (
                        <div
                          key={item.id}
                          className="group/card relative w-[220px] md:w-[260px] shrink-0 snap-start aspect-[4/5] overflow-hidden rounded-[1.5rem] border border-[var(--border)] bg-[var(--surface-soft)] shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:-translate-y-1.5 hover:shadow-[0_20px_40px_rgba(116,12,8,0.08)] transition-all duration-500 flex flex-col"
                        >
                          <div className="relative flex-1 w-full overflow-hidden border-b border-[var(--border)]">
                            {/* Hover Overlay Effect (kept intact and placed over image via z-index) */}
                            ...
                            <Image
                              src={item.thumbnailLink?.replace('=s220', '=s800') || ''}
                              alt={item.name || 'Archive Memory'}
                              fill
