...
export default function ContestsHubPage() {
  ...
  const getBentoClasses = (index: number) => {
    // The base class applies the carousel widths/snaps on mobile, and resets on md:
    const carouselBase = "w-[85vw] sm:w-[320px] md:w-auto shrink-0 snap-start md:snap-none";
    
    if (index === 0) return `${carouselBase} md:col-span-2 md:row-span-2 flex flex-col justify-end min-h-[400px] md:min-h-[480px]`;
    if (index === 1) return `${carouselBase} md:col-span-2 md:row-span-1 flex flex-col justify-center min-h-[260px] md:min-h-[220px]`;
    return `${carouselBase} md:col-span-1 md:row-span-1 flex flex-col justify-between min-h-[260px] md:min-h-[220px]`;
  };
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col items-center pt-24 pb-20 px-4 sm:px-6 lg:px-8 font-sans overflow-hidden bg-[var(--background)]">
      ...
      <main className="relative z-10 w-full max-w-[1140px] mx-auto flex flex-col">
        ...
        {/* Header Section */}
        <div className="text-center mb-10 md:mb-16 animate-in fade-in slide-in-from-top-6 duration-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 lg:gap-8 w-full">
          ...
          {/* The magic 'md:contents' wrapper converts this into a carousel on mobile only */}
                ...
                <React.Fragment key={contest.id}>
                  {isActive ? (
                    ...
                    >
                      ...
                      {/* UPGRADED: Ultra-Subtle Watermarks with Gentle Tactile Press (No more aggressive pulse) */}
                      {contest.isTextIcon ? (
                        <div className="absolute -right-6 -bottom-4 md:-right-8 md:-top-8 text-[14rem] md:text-[20rem] leading-none font-serif text-[var(--color-mace-gold)] opacity-[0.02] md:opacity-0 md:group-hover:opacity-[0.02] group-active:-translate-x-1 group-active:-translate-y-1 md:group-hover:-translate-x-1 md:group-hover:-translate-y-1 transition-all duration-1000 ease-out pointer-events-none select-none">
                          {contest.iconSymbol}
                        </div>
                      ) : (
                        contest.icon && <contest.icon className="absolute -right-6 -bottom-6 md:-right-12 md:-top-12 w-56 h-56 md:w-96 md:h-96 text-[var(--color-mace-gold)] opacity-[0.02] md:opacity-0 md:group-hover:opacity-[0.02] group-active:-translate-x-1 group-active:-translate-y-1 md:group-hover:-translate-x-1 md:group-hover:-translate-y-1 transition-all duration-1000 ease-out pointer-events-none" />
                      )}
                    ...
                    /* --- THE LOCKED CONTESTS (VERTICAL CAROUSEL CARDS) --- */
                    /* On mobile carousel, vertical cards look far better than smashed list rows */
                    <div className={`relative rounded-[1.5rem] md:rounded-[2.5rem] bg-[var(--surface-soft)] border border-[var(--border)]/60 p-6 md:p-8 lg:p-10 opacity-90 grayscale-[20%] animate-in fade-in slide-in-from-bottom-4 flex flex-col items-start justify-between active:scale-[0.98] transition-transform duration-500 select-none ${animationDelay} ${bentoLayout}`}>
                      
                      <div className="w-full flex flex-col items-start gap-4 md:gap-0">
                        <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl md:rounded-[1.5rem] bg-[var(--background)] border border-[var(--border)] flex items-center justify-center flex-shrink-0 shadow-sm md:mb-6">
                          {contest.icon && <contest.icon className="w-6 h-6 md:w-6 md:h-6 text-[var(--color-mace-stone)]" strokeWidth={1.5} />}
                        </div>
                        
                        <div className="flex-1 mt-2 md:mt-0">
                          <p className="text-[9px] md:text-[10px] font-bold uppercase tracking-[0.25em] text-[var(--text-muted)]/60 mb-1.5 flex items-center gap-2">
                            {contest.tag} <span className="text-[var(--border)] mx-1">•</span> Locked
                          </p>
                          <h2 className="text-xl md:text-2xl font-serif font-bold text-[var(--text-muted)]/70 mb-2 tracking-tight">
                            {contest.title}
                          </h2>
                          {index === 1 && (
                            <p className="text-sm text-[var(--text-muted)]/60 font-medium max-w-sm leading-relaxed hidden md:block">
                              {contest.description}
                            </p>
                          )}
                        </div>
                      ...
                      <div className="mt-8 md:mt-auto w-full">
                        {/* Unified Mobile/Desktop Lock Pill */}
                        <div className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-transparent border border-[var(--border)] text-[var(--color-mace-stone)] rounded-full text-[10px] md:text-[11px] font-bold uppercase tracking-[0.25em] cursor-not-allowed">
                          <Lock className="w-3.5 h-3.5 md:w-4 md:h-4" strokeWidth={2} /> Locked
                        </div>
                      </div>
                    </div>
                  )}
