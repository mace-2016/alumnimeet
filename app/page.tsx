...
import { ArrowUpRight, MessageCircle, MapPin, Calendar, Trophy, Sparkles, Clock, Film } from "lucide-react";
  ...
  return (
    ...
    >
     ...
     <div className="mt-6 lg:mt-8 w-full">
        <div className="group relative w-full bg-white/70 backdrop-blur-xl rounded-[2rem] md:rounded-[2.5rem] border border-white/50 p-6 md:p-10 lg:p-12 flex flex-col overflow-hidden shadow-[0_8px_30px_rgba(116,12,8,0.02)] transition-all duration-700">
            ...
            <div className="flex items-center gap-4 md:gap-6 lg:gap-8">
              ...
              <div>
                <div className="mb-1 md:mb-2 flex flex-wrap items-center gap-3 md:gap-4 transition-transform duration-700 group-hover:translate-x-2">
                  <h3 className="font-serif text-2xl md:text-3xl lg:text-4xl font-medium tracking-tight text-[var(--color-mace-crimson)]">
                    The <span className="bg-gradient-to-r from-[var(--color-mace-gold)] via-[var(--color-mace-rust)] to-[var(--color-mace-gold)] bg-[length:200%_auto] bg-clip-text text-transparent font-semibold" style={{ animation: "shimmer 6s linear infinite" }}>Archive</span>
                  </h3>
                ...
                <p className="max-w-md text-sm md:text-base lg:text-lg font-medium text-[var(--text-muted)] leading-relaxed transition-transform duration-700 group-hover:translate-x-2">
                  A decade of memories, perfectly preserved.
                </p>
              </div>
            </div>

            <Link href="/gallery" aria-label="Enter the Master Archive" className="hidden md:flex items-center gap-3 text-[11px] font-black uppercase tracking-[0.3em] text-[var(--text-muted)] hover:text-[var(--color-mace-gold)] transition-colors duration-500 relative z-10 shrink-0 group/btn">
              Enter Vault
              <div className="w-10 h-10 rounded-full bg-[var(--surface-soft)] border border-[var(--border)] flex items-center justify-center group-hover/btn:bg-[var(--color-mace-gold)]/10 group-hover/btn:border-[var(--color-mace-gold)]/20 transition-all duration-500">
                <ArrowUpRight className="h-5 w-5 text-[var(--color-mace-crimson)] group-hover/btn:text-[var(--color-mace-gold)] group-hover/btn:translate-x-0.5 group-hover/btn:-translate-y-0.5 transition-all" strokeWidth={1.5} aria-hidden="true" />
              </div>
            </Link>
          </div>

          {/* MINI SNAP-SCROLL GALLERY (Matches your actual Gallery structure) */}
          <div className="relative z-10 w-full -mx-6 px-6 md:mx-0 md:px-0">
            <div className="absolute top-0 right-0 bottom-0 w-12 md:w-24 bg-gradient-to-l from-white/70 to-transparent z-10 pointer-events-none" />
            
            <div className="flex overflow-x-auto hide-scrollbar gap-4 md:gap-6 pb-8 snap-x snap-mandatory">
              {RIBBON_IMAGES.slice(0, 5).map((img, idx) => (
                <div key={idx} className="group/card relative w-[200px] md:w-[240px] shrink-0 snap-start aspect-[4/5] overflow-hidden rounded-[1.5rem] border border-[var(--border)] bg-[var(--surface-soft)] shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:-translate-y-1.5 hover:shadow-[0_20px_40px_rgba(116,12,8,0.08)] transition-all duration-500 flex flex-col cursor-pointer">
                  <div className="relative flex-1 w-full overflow-hidden border-b border-[var(--border)]">
                    <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_30%,rgba(236,187,26,0.15),transparent_60%)] opacity-0 group-hover/card:opacity-100 transition-opacity duration-700 z-10 pointer-events-none" />
                    <Image
                      src={img.src}
                      alt={img.caption}
                      fill
                  ...
                  <div className="p-4 bg-white relative z-20">
                    ...
                    <h4 className="font-serif text-sm md:text-base text-[var(--color-mace-crimson)] truncate">
                      {img.caption}
                    </h4>
                  </div>
                </div>
              ...
              {/* VIEW ALL CARD */}
              <Link href="/gallery" className="group/seeall relative w-[200px] md:w-[240px] shrink-0 snap-start aspect-[4/5] rounded-[1.5rem] border border-[var(--border)] bg-white shadow-sm hover:border-[var(--color-mace-gold)]/40 hover:shadow-[0_18px_40px_rgba(116,12,8,0.08)] transition-all duration-500 flex flex-col items-center justify-center p-6 text-center">
                <div className="w-12 h-12 rounded-full bg-[var(--surface-soft)] border border-[var(--border)] flex items-center justify-center mb-4 group-hover/seeall:bg-[var(--color-mace-gold)]/10 transition-colors">
                  <ArrowUpRight className="w-6 h-6 text-[var(--color-mace-crimson)] group-hover/seeall:text-[var(--color-mace-gold)]" strokeWidth={1.5} />
                </div>
                <h4 className="font-serif text-lg text-[var(--color-mace-crimson)] mb-2">
                  Open Vault
                </h4>
